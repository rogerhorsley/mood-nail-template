...
export const App: AppInterface = {
  ...
  initPage() {
    ...
    if (this.store && this.blueprint) {
      // Find state by ID or create from URL params
      if (stateId) {
        const state = this.blueprint.states.find((s) => s.id === stateId);
        if (state) {
          this.currentState = state;
        } else {
          this.currentState = this.createStateFromParams(pageName, urlParams);
        }
      } else {
        this.currentState = this.createStateFromParams(pageName, urlParams);
      }
      ...
      .then(([blueprint, store, theme]) => {
        ...
        if (stateId) {
          ...
          if (state) {
            ...
            // Create dynamic state from URL params
            this.currentState = this.createStateFromParams(pageName, urlParams);
          }
        } else {
          // Create dynamic state from URL params
  ...
  transitionTo(targetStateId: string, runtimeParams?: Record<string, any>) {
    const state = this.blueprint?.states.find((s) => s.id === targetStateId);
    if (!state) {
      console.error(`State ${targetStateId} not found`);
      return;
    }

    const currentStateId = this.currentState ? this.currentState.id : null;
